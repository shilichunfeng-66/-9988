# Research Writing Skill **Pro** — 论文写作专业版

> 🎓 论文写作专业版：在 `research-writing`（30 个 Prompt + 中文论文生成子流程，**内容完全照搬**）基础上，将**深度增强设为默认启用**——三轮审核-返稿-重修、科研化描述、数据比对与性能支撑、论文达标度自评与提升路线。
>
> **版本**: Pro 1.0（基础 research-writing 3.0）| **License**: MIT | **语言**: 中文/英文
>
> :lock: **乐动微伴工作室内部专属，禁止外传。**
...
| 维度 | research-writing（标准版） | research-writing-pro（本 skill） |
|------|--------------------------|--------------------------------|
| 30 个 Prompt / 中文论文生成子流程 | ✓ | ✓（完全照搬） |
| 三轮审核-返稿-重修 | 需用户要求 | **默认执行**（学位论文评审专家 / 期刊审稿人 / 方法学 / 领域双视角） |
| 科研化描述 | 需用户要求 | **默认叠加** |
| 数据比对与性能支撑 | 按需 | **默认检查与补充**（理工科优先，三线表 ≥4） |
| 论文达标度自评（五维 + 逻辑贯通两查 + 提升路线） | 无 | **默认给出** |

> 详见 SKILL.md 顶部「🔷 专业版默认增强」。
